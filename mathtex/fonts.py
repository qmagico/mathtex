...
from mathtex.afm import AFM

# Hack until we have a proper data dir function
import os
...
class BakomaFonts(Fonts):
    basepath = os.path.join(data_dir, 'fonts', 'bakoma')
    ...
    def __init__(self):
        ...
        for key, val in self._fontmap.iteritems():
            self.fontmap[key] = val
            self.fontmap[val] = val
    ...
    def _get_info (self, fontname, font_class, sym, fontsize, dpi):
        ...
        glyph = sym
        offset = self._get_offset(font, num, fontsize, dpi)

        scale = 0.001 * fontsize * dpi/72.0

        print symbol_name, fontsize, font.get_height_char(glyph)

        xmin, ymin, xmax, ymax = [val * scale
                                  for val in font.get_bbox_char(chr(num))]
        metrics = Bunch(
    ...
    def _get_glyph(self, fontname, font_class, sym, fontsize):
        ...
        if fontname in self.fontmap and sym in latex_to_bakoma:
            ...
            try:
                ...
                symbol_name = cached_font.font.get_name_char(num)
        elif len(sym) == 1:
            slanted = (fontname == "it")
            try:
                ...
                num = ord(sym)
                if num is not None:
                    symbol_name = cached_font.get_name_char(num, True)
    ...
    def get_xheight(self, font, fontsize, dpi):
        ...
        return cached_font.get_xheight() * 0.001 * fontsize * dpi/72.0
